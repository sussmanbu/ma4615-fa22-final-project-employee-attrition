...
CO2_clean_1 %>% filter(!Country %in% c('World','European Union (27)')) %>% mutate(developed = ifelse(Country %in% all_developed, 'Developed', 'Developing')) %>% filter(CO2emission != 0) %>% group_by(Country) %>% 
  transmute(meanCO2 = mean(CO2emission,rm.na = TRUE), developed = developed) %>%
  distinct(Country, meanCO2, developed) %>%
  ggplot(aes(developed, meanCO2,color = developed)) + geom_boxplot() + ylim(0,100) +
