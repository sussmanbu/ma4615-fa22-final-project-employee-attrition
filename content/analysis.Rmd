...
```
```{r Temp-Change}
source = read_csv('~/Desktop/Environment_Temperature_change_E_All_Data/Environment_Temperature_change_E_All_Data.csv')
...
world_map = st_read('~/Desktop/worldmap/ne_50m_admin_0_countries.shp', quiet = TRUE)
...
total_change <- function(x, y){
  x = x - 1958
  y = y - 1958
  Pic_data = TemChange_Cont %>% as.tibble() %>% select(x:y) %>%
    mutate(total = apply(., 1, sum, na.rm = TRUE)) %>%
    cbind(TemChange_Cont[, 1]) %>% select(Area, total) %>%
    cbind(TemChange_Cont[, "geometry"]) %>% st_as_sf()
  Pic = tm_shape(world_map) + tm_borders(lwd = 2) +
    tm_shape(Pic_data) + 
    tm_polygons(col = 'total', palette = 'Blues', midpoint = NA, n = 7)
  return(Pic)
}
