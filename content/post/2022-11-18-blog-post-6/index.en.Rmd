...
server <- function(input, output) {
  output$scatter <- renderPlot({
    ...
    # Initializes the line plot
    table <- ggplot() + labs(x = "Year", y = "CO2 Emission", color = "Country", title = "Progression of CO2 Levels")
    
    
    # Appends a line to plot for each country selected
    for (country in input$country) {
      line <- df %>% 
        filter(Country == country & Year >= input$year) %>% 
        geom_line(mapping = aes(x = Year, y = CO2emission, group = 1, colour = as.factor(Country)))
